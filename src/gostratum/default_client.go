...
var walletRegex = regexp.MustCompile("cas:[a-z0-9]+")

func CleanWallet(in string) (string, error) {
	// Check if the address is already in a valid format, decode it accordingly
	_, err := util.DecodeAddress(in, util.Bech32PrefixPyrin) // Assuming util.DecodeAddress can handle CAS addresses too
	if err == nil {
		return in, nil // The address is good to go
	}
	
	// Ensure the address starts with the correct prefix
	if !strings.HasPrefix(in, "cas:") {
		in = "cas:" + in // Prepend "cas:" if not present
	}

	// Match the address against the regex pattern to validate it
	if walletRegex.MatchString(in) {
		// Since CAS address has the "cas:" prefix and 66 characters, we slice till 70 characters
		// This assumes the "cas:" prefix is required in the address returned
		// If the prefix is not needed, adjust the slicing accordingly
		if strings.HasPrefix(in, "cas:") && len(in) > 66 {
			return in[:70], nil // Return the first 70 characters which includes the "cas:" prefix
		}
		return in, nil // Return the full match if it's less than 70 characters
	}

	// Return an error if the address doesn't match the expected format
	return "", errors.New("unable to coerce wallet to valid CAS address")
}
